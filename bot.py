...
def init_db():
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_topup_status_user ON topup_requests (status, user_id)")
...
def db_get_setting(key, default_val=""):
    now = time.monotonic()
    cached_at = SETTINGS_CACHE_LOADED_AT.get(key, 0.0)
    if key in SETTINGS_CACHE and (now - cached_at) < SETTINGS_CACHE_TTL:
    ...
    try:
        ...
        row = cursor.fetchone()
        value = row[0] if row else default_val
        SETTINGS_CACHE[key] = value
        SETTINGS_CACHE_LOADED_AT[key] = now
        return value
    finally:
        cursor.close()
        conn.close()
...
def db_set_setting(key, value):
    ...
    try:
        ...
        ''', (str(key), str(value)))
        conn.commit()
        SETTINGS_CACHE[str(key)] = str(value)
        SETTINGS_CACHE_LOADED_AT[str(key)] = time.monotonic()
    finally:
        cursor.close()
        conn.close()
...
def db_get_user(user_id, username_val=""):
    ...
    cached = USER_CACHE.get(str_id)

    if cached is not None:
        if username_val and username_val != cached.get("username", ""):
            conn = get_db()
            cursor = conn.cursor()
            try:
                cursor.execute("UPDATE users SET username = %s WHERE user_id = %s", (username_val, str_id))
                conn.commit()
                cached["username"] = username_val
            finally:
                cursor.close()
                conn.close()
        return cached
    ...
    try:
        cursor.execute(
            "SELECT credit, ref_count, ref_credit, projects, is_blocked, username "
            "FROM users WHERE user_id = %s", (str_id,)
        )
        row = cursor.fetchone()
        ...
        if not row:
            ...
            cursor.execute(
                "INSERT INTO users (user_id, username, credit, ref_count, ref_credit, projects, is_blocked) "
                "VALUES (%s, %s, %s, %s, %s, %s, %s)",
                (str_id, username_val, welcome_coins, 0, 0, default_projects, 0)
            )
            conn.commit()
            user_data = {
                "user_id": str_id, "username": username_val, "credit": welcome_coins,
                "ref_count": 0, "ref_credit": 0, "projects": [], "is_blocked": 0
            }
        else:
...
def db_save_user(u_data):
    ...
    try:
        cursor.execute('''
            UPDATE users
            SET username = %s, credit = %s, ref_count = %s, ref_credit = %s,
                projects = %s, is_blocked = %s
            WHERE user_id = %s
        ''', (
            u_data.get("username", ""), u_data["credit"], u_data["ref_count"],
            u_data["ref_credit"], json.dumps(u_data["projects"], ensure_ascii=False),
            int(u_data.get("is_blocked", 0)), str_id
        ))
        conn.commit()
        USER_CACHE[str_id] = u_data
    finally:
        cursor.close()
        conn.close()
...
def db_get_all_users():
    ...
    now = time.monotonic()
    if USER_CACHE and (now - USER_CACHE_LOADED_AT) < USER_CACHE_TTL:
        return USER_CACHE
    ...
    try:
        cursor.execute(
            "SELECT user_id, credit, ref_count, ref_credit, projects, is_blocked, username FROM users"
        )
...
# 📱 Keyboards with ReplyKeyboardMarkup and button styles/colors (primary, success, danger)
def get_user_keyboard():
    kb = [
        [KeyboardButton("⚙️ Setup", style="primary"), KeyboardButton("👤 Profile", style="success")],
        [KeyboardButton("🛠️ Settings", style="primary"), KeyboardButton("💰 Top-up", style="success")],
        [KeyboardButton("📋 Order List", style="primary"), KeyboardButton("🎧 Support", style="success")],
        [KeyboardButton("👥 Refer & Earn", style="danger")]
    ]
    return ReplyKeyboardMarkup(kb, resize_keyboard=True)

def get_admin_keyboard():
    return ReplyKeyboardMarkup([
        [KeyboardButton("📊 Admin Dashboard", style="primary")],
        [KeyboardButton("👥 Users Report", style="success"), KeyboardButton("📢 Send SMS", style="danger")],
        [KeyboardButton("👤 Search User", style="success"), KeyboardButton("🏠 Main Menu", style="danger")]
    ], resize_keyboard=True)

def cancel_keyboard():
    return ReplyKeyboardMarkup([[KeyboardButton("❌ Cancel", style="danger")]], resize_keyboard=True)
...
def build_setup_dashboard_markup_and_text(context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Channel Link", callback_data="setup_btn_channel", style="primary")],
        [InlineKeyboardButton("👀 Views", callback_data="setup_btn_views", style="danger"), InlineKeyboardButton("❤️ Reacts", callback_data="setup_btn_reacts", style="success")],
        [InlineKeyboardButton("✅ Save Setup", callback_data="setup_btn_save", style="primary")]
    ])
...
async def project_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("p_togreact_"):
        ...
        if 0 <= idx < len(projects):
            proj = projects[idx]
            kb = [
                [InlineKeyboardButton("📢 Edit Channel", callback_data=f"fe_{idx}_channel", style="primary")],
                [InlineKeyboardButton("📊 Edit Reaction Count", callback_data=f"fe_{idx}_count", style="primary")],
                [InlineKeyboardButton("👁️ Edit Views Count", callback_data=f"fe_{idx}_views", style="primary")],
                [InlineKeyboardButton("🔙 Back", callback_data="p_back", style="danger")]
            ]
...
async def show_my_projects(message_obj, user_id):
    ...
    for idx, p in enumerate(projects):
        ...
        kb = [
            [InlineKeyboardButton(btn_react_text, callback_data=f"p_togreact_{idx}", style="success"), InlineKeyboardButton(btn_view_text, callback_data=f"p_togview_{idx}", style="success")],
            [InlineKeyboardButton("✏️ Edit Settings", callback_data=f"p_edit_{idx}", style="primary")]
        ]
        p_text = (
...
def build_user_card_text_and_markup(uid):
    ...
    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("➕ Add", callback_data=f"uact_add_{uid}", style="success"),
            InlineKeyboardButton("➖ Deduct", callback_data=f"uact_deduct_{uid}", style="danger")
        ],
        [
            InlineKeyboardButton(f"{ban_btn_text}", callback_data=f"uact_ban_{uid}", style="danger"),
            InlineKeyboardButton("📋 Orders", callback_data=f"uact_orders_{uid}", style="primary")
        ]
    ])
...
async def admin_dashboard_inline_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "dash_bot_orders":
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("✏️ Edit API Key", callback_data="edit_setting_smm_api_key", style="primary")]])
        ...
        for req in pending_topups:
            ...
            kb = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Approve", callback_data=f"topup_approve_{rid}", style="success"),
                    InlineKeyboardButton("❌ Reject", callback_data=f"topup_reject_{rid}", style="danger")
                ]
            ])
            try:
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✏️ Edit Reaction Service ID", callback_data="edit_setting_smm_service_id", style="primary")],
            [InlineKeyboardButton("✏️ Edit View Service ID", callback_data="edit_setting_smm_view_service_id", style="primary")]
        ])
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("✏️ Edit View Coin Rate", callback_data="edit_setting_view_coin_rate", style="primary")],
            [InlineKeyboardButton("✏️ Edit Dollar Rate ($1 = ? Coins)", callback_data="edit_setting_dollar_rate", style="danger")]
        ])
        return await query.message.reply_text(
            f"💡 **কয়েন রেট সেটিংস:**\n───────────────────\n"
